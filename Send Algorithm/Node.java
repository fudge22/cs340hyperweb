...
//		nodes.get(secondId).setParent(firstId);
	 ...
	 */
	public static void sendFirstNode(Node startNode, Node endNode) {
		...
		int commonFoldID = foldID.getNumberBitsInCommon(endNode.getWebID());
		int commonNeighborID = neighborID.getNumberBitsInCommon(endNode.getWebID());
		
		if(commonNeighborID > commonFoldID) {
			//go through the neighbors
			sendNode(startNode, endNode);
		} else {
			//go to the fold and then go through the neighbors
			sendNode((Node) startNode.getFold(), endNode);
		}
