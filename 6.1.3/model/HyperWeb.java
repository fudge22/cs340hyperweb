...
//		PortNumber.setApplicationsPortNumber(new PortNumber(portNum));
	...
	public  Node addNode() {
		...
		else if (insertPoint.getWebId() == 0){
			//check if it has neighbors
			if (insertPoint.getNeighborList().size() == 0){
...
//			lastPrintState = currentPrintState;
	...
	public  Node getRandomNode(GlobalObjectId globalObjectId2) {
		...
		if (hw != null && hw.nodes != null && hw.nodes.size() != 0){
			for (WebID id : hw.nodes.keySet()) {
				insertPointID = id;
				break;
			}
			
			insertPoint = getNearNode(new WebID( randomInsertionPoint, 1),insertPointID).getNode();
