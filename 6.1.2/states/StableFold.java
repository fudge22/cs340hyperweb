...
public class StableFold extends FoldState implements Serializable{
	...
	public void updateFoldOf(Node parent, Node child) {
		...
		Node parentsFold = parentsFoldId.getNode();
	...
	public void removeFoldsOf(Node d) {
			...
			else if (parent.getFoldState() instanceof UnstableSF) //should always be either stable or unstable surrogate fold state
			{
				parent.setFoldID(parent.getSurrogateFoldID());
				parent.removeSurFold(parent.getSurrogateFoldID());
				Node parentFold = parent.getFoldID().getNode();
			...
			} else if (parent.getFoldState() instanceof UnstableISF) {
