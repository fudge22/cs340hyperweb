...
//		PortNumber.setApplicationsPortNumber(new PortNumber(portNum));
	...
	public  WebID findNode(int webID)
	{
		for(Node test : nodes.values())
		{
			if(test.getWebID().getId() == webID)
			{
				return test.getWebID();
			}
		}
		return null;
	}
...
//				currentPrintState += n1.getNeighborDown().toString() + "\n";
//				
...
//		}
	...
	public static GlobalObjectId getGlobalObjectId() {
		try {
...
//    		hashTable = new Hashtable<LocalObjectId, Object>();
	...
	public HyperwebFaceProxy getNextSeg() {
		return next;
	}
