...
public class DatabaseTests {
	...
	public static void setUpBeforeClass() throws Exception {
		...
		try {
			Node node1 = new Node(new WebID(1), 1);
			Node node2 = new Node(new WebID(2), 1);
			Node node3 = new Node(new WebID(3), 1);
			db.startTransaction();
			db.getDatabaseAccessor().addNode(node1);
			db.getDatabaseAccessor().addNode(node2);
			db.getDatabaseAccessor().addNode(node3);
			db.getDatabaseAccessor().addNode(new Node(new WebID(4), 1));
			db.getDatabaseAccessor().addNode(new Node(new WebID(5), 1));
			db.getDatabaseAccessor().addNode(new Node(new WebID(6), 1));
			db.getDatabaseAccessor().addNode(new Node(new WebID(7), 1));
			
			ArrayList<Integer> neighborList = new ArrayList<Integer>();
			neighborList.add(4);
			neighborList.add(5);
			db.getDatabaseAccessor().addNeighbors(node2.getWebID().getValue(), neighborList);
	...
	public void testAddNode() {
		try {
			Node node = new Node(new WebID(8), 1);
			db.startTransaction();
	...
	public void testDeleteNode() {
		try {
			Node node = new Node(new WebID(1), 1);// dummy node with webID of 1
			db.startTransaction();
